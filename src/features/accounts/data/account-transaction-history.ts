...
const fetchAccountTransactionAtomBuilder = (store: JotaiStore, address: Address, pageSize: number, nextPageToken?: string) => {
...
export const useFetchNextAccountTransactionPage = (address: Address) => {
  ...
  return useMemo(() => {
    return (pageSize: number, nextPageToken?: string) => fetchAccountTransactionAtomBuilder(store, address, pageSize, nextPageToken)
  }, [store, address])
}
